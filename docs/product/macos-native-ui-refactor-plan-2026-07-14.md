...
> 当前显示器或聚焦窗口正在被 OpenWhisper 的 AI 输入流程使用。
...
`packaging/assets/OpenWhisperLogoSource.png`
...
| Logo / Icon | `OpenWhisperVisualSystem.swift`、`scripts/render_app_icon.swift`、`scripts/package_app.sh` |
...
1. 从 `/Applications/OpenWhisper.app` 启动，不从 `dist` 启动。
...
- 最终正常启动的 `/Applications/OpenWhisper.app` 保持运行，供产品复核。
...
- OpenWhisper 使用标准 `.regular` 激活策略并继续提供菜单栏入口；Dock 始终显示品牌
