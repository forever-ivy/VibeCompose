# OpenWhisper Community Skills 核心化下一步计划

> 日期：2026-07-15  
> 状态：Phase 0–3 工程实现完成；Phase 4 内容、研究手册和执行工具包就绪，招募尚未开始，Community Pilot 与 Registry Go/No-Go 待真实用户证据  
> 产品范围：macOS 原生 OpenWhisper、Community Skills、Context、Preview、History 与安全输出  
> 商业化范围：明确排除；Phase 0 删除现有商业化设计与实现，不保留隐藏付费门禁  
> 当前基线：`0.1.0 Alpha`  
> 公共创作格式：Agent Skills 标准目录；Legacy `.openwhisperskill` v1 仅保持兼容  
...
OpenWhisper 的下一阶段不应继续横向增加设置项，也不应把 Community Skills 留在
“AI Polish 的高级选项”里。Community Skills 应成为用户理解 OpenWhisper 的主要方式：
...
2. **可移植标准**：同一个 Agent Skill 目录可以被不同 Host 理解，OpenWhisper 不制造孤岛格式。
...
8. **开放格式、受限运行**：接受标准内容，但只执行 OpenWhisper 的声明式兼容子集。
...
- 不兼容脚本/工具：说明“OpenWhisper 不会执行这些内容”；若 Skill 依赖它们才能成立，则拒绝安装或标记不可运行。
...
Undo 只在 OpenWhisper 能验证“目标当前仍包含刚刚插入的准确结果”时执行反向替换；无法验证时不自动修改目标，而是提供复制原文。Undo 状态必须与 History 结果一致。
...
- Legacy `.openwhisperskill` 的历史版本、授权与回滚继续有效。
...
9. 使用 `/Applications/OpenWhisper.app` 做真实 installed-app 流程。

不得从 `dist/OpenWhisper.app` 直接启动进行权限或交互证明。
...
完成验收后必须重新启动正常的 `/Applications/OpenWhisper.app` 并保持菜单栏 App 运行；如果最终工作是 Skill Library/Creator，则在实际可行时将对应窗口留给产品所有者检查。
...
| “Agent Skills 兼容”被误解 | 用户以为 OpenWhisper 会执行脚本或工具 | 安装兼容性报告明确“可读取/忽略/拒绝”，文案不承诺等价运行 |
...
当且仅当以下条件全部满足，才能称为“Community Skills 已成为 OpenWhisper 的产品核心”：
...
- 关闭任务时 `/Applications/OpenWhisper.app` 以正常菜单栏状态运行并留给产品所有者复核。
