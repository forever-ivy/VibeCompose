...
| 404 或 Schema 不兼容 | 上游契约变化 | 停止把能力视为健康，要求更新应用 |
...
5. 等待上游恢复，或安装经过验证的 VibeCompose 更新；
6. 不再需要 Recovery 时，删除失败音频或全部本地数据。
...
私有 Alpha 仍刻意不写入生产 URL 和公钥。公开签名构建必须具备独立生产签名密钥、
永久 HTTPS 策略托管、初始签名策略及安装版事故演练。
