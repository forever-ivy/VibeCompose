...
| OW-AUD-005 录音上限/大文件内存 | 部分关闭 | `AudioRecorder` 使用 deadline task 硬停止；停止/取消清理 deadline；配置变化会重新设置上限 | `ChatGPTTranscriber` 仍先整文件 `Data(contentsOf:)`，再检查 25 MB；需先查 metadata 并采用流式 multipart |
...
| OW-AUD-009 技术字面量归一化 | 部分关闭 | Prompt 要求保留 URL/path/command 等；模糊术语匹配跳过受保护 literal range | 仍默认执行简体化和全角标点；需建立 literal tokenizer/round-trip 测试并支持用户语言偏好 |
| OW-AUD-010 MainActor/旧回调污染 | 部分关闭 | 协调器使用 `activeSessionID`，取消后迟到 pipeline 结果不能写入新状态；Retry 有独立生命周期 | `waitForPasteTarget` 仍在 MainActor 同步等待；应改为可取消 async；继续统一 HUD callback generation |
...
1. 先关闭 OW-AUD-008：严格 env parser、签名、公证、哈希、原子安装和回滚；
2. 把音频上传改成 metadata-first + streaming；
3. 把粘贴目标等待移出 MainActor，并区分事件发送与确认插入；
4. 完成 clean TCC Onboarding 和真实应用矩阵；
5. 完成政策文档、Updater、诊断导出脱敏和发布前复核。
