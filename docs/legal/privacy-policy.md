...
If Pro licensing is configured, the signed activation receipt and a random,
app-specific License Device ID are stored in separate macOS Keychain items.
Their services are `app.openwhisper.mac.LicenseReceipt` and
`app.openwhisper.mac.LicenseDevice`.
The Device ID is generated by OpenWhisper and is not derived from a hardware
serial number, Apple ID, account email, or document content. The receipt may
contain a license identifier, activation identifier, edition, enabled feature
list, verification dates, eligible build, and device limit. It does not need
to contain an account email or payment details.
...
session, optional Recovery API key, signed Pro receipt, and random License
Device ID; verifies license receipts with an embedded Ed25519 public key; and
uses owner-only local file permissions where supported, bounded retention,
HTTPS endpoint validation, redirect rejection, and conservative paste
behavior. The private signing key is not included in the app. No security
