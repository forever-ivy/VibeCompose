...
capture_mode() {
  ...
  temporary_debug="$(mktemp "${TMPDIR:-/tmp}/vibecompose-feedback-debug.XXXXXX")"
  temporary_log="$(mktemp "${TMPDIR:-/tmp}/vibecompose-feedback-log.XXXXXX")"
  ...
  if [[ "$capture_png" == "1" ]]; then
    temporary_png="$(mktemp "${TMPDIR:-/tmp}/vibecompose-feedback-png.XXXXXX")"
    rm -f "$temporary_png"
    launch_args+=(
...
# VibeCompose Feedback Mode Acceptance
