...
ARCH="$(uname -m)"
ZIP="$ROOT/dist/${VIBECOMPOSE_APP_NAME}-${VIBECOMPOSE_VERSION}-macos-${ARCH}.zip"
MANIFEST="${VIBECOMPOSE_RELEASE_MANIFEST_PATH:-$ROOT/dist/release-manifest.json}"
CHANNEL="${VIBECOMPOSE_SPARKLE_CHANNEL:-stable}"
KEY_ACCOUNT="${VIBECOMPOSE_SPARKLE_KEY_ACCOUNT:-vibecompose}"
PRIVATE_KEY_FILE="${VIBECOMPOSE_SPARKLE_PRIVATE_KEY_FILE:-}"
PUBLIC_KEY="${VIBECOMPOSE_SPARKLE_PUBLIC_ED_KEY:-}"

case "$CHANNEL" in
  stable|beta|alpha)
    ;;
  *)
    echo "VIBECOMPOSE_SPARKLE_CHANNEL must be stable, beta, or alpha." >&2
    exit 1
    ;;
esac
...
APPCAST="${VIBECOMPOSE_SPARKLE_APPCAST_PATH:-$APPCAST}"
...
[[ "$PUBLIC_KEY" =~ ^[A-Za-z0-9+/]{43}=$ ]] || {
  echo "VIBECOMPOSE_SPARKLE_PUBLIC_ED_KEY must contain the matching 32-byte base64 public key." >&2
  exit 1
}
...
RELEASE_NOTES="$ROOT/docs/releases/v${VIBECOMPOSE_VERSION}.md"
if [[ -f "$RELEASE_NOTES" && ! -L "$RELEASE_NOTES" ]]; then
  /usr/bin/ditto \
    "$RELEASE_NOTES" \
    "$WORK_DIR/${VIBECOMPOSE_APP_NAME}-${VIBECOMPOSE_VERSION}-macos-${ARCH}.md"
fi

GENERATOR_ARGS=(
  --download-url-prefix "$DOWNLOAD_URL_PREFIX"
  --link "https://github.com/$VIBECOMPOSE_REPOSITORY"
  --maximum-versions 5
  --maximum-deltas 3
  --versions "$VIBECOMPOSE_BUILD"
  -o appcast.xml
)
if [[ "$CHANNEL" != "stable" ]]; then
...
if [[ -n "$PRIVATE_KEY_FILE" ]]; then
  [[ -f "$PRIVATE_KEY_FILE" && ! -L "$PRIVATE_KEY_FILE" ]] || {
    echo "VIBECOMPOSE_SPARKLE_PRIVATE_KEY_FILE must be a regular, non-symlink file." >&2
    exit 1
  }
