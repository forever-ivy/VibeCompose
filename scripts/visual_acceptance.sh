...
capture_state() {
  ...
  local log_file="$OUT_DIR/vibecompose-overlay-demo-$state-$profile.log"
  ...
  launch_log_file="$(mktemp "${TMPDIR:-/tmp}/vibecompose-overlay-demo-$state-$profile.XXXXXX")"
  self_capture_file="$(mktemp "${TMPDIR:-/tmp}/vibecompose-overlay-snapshot-$state-$profile.XXXXXX")"
  rm -f "$self_capture_file"
  if [[ -n "$followup_output_file" ]]; then
    followup_capture_file="$(mktemp "${TMPDIR:-/tmp}/vibecompose-overlay-followup-$state-$profile.XXXXXX")"
...
capture_preview() {
  ...
  local launch_log_file
  local self_capture_file
  launch_log_file="$(mktemp "${TMPDIR:-/tmp}/vibecompose-preview-demo.XXXXXX")"
  self_capture_file="$(mktemp "${TMPDIR:-/tmp}/vibecompose-preview-snapshot.XXXXXX")"
  rm -f "$self_capture_file"
  ...
  wait "$OPEN_PID" >/dev/null 2>&1 || true
  OPEN_PID=""
  cp "$launch_log_file" "$OUT_DIR/vibecompose-preview-demo.log"
...
- Evidence:
  ...
  - \`vibecompose-overlay-demo-*.log\`
  - \`vibecompose-preview-demo.log\`
