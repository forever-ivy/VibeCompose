...
final class AppCoordinator {
    ...
    /// Who initiated a config commit — Settings saves do not re-push into the
    /// open draft; external mutators rebase the draft via 3-way merge.
    private enum ConfigCommitSource {
        case settings
        case external
    }
    ...
    // MARK: - D4 Voice Refinement in Preview
    /// True while a PreviewPresenting call is suspended (panel is visible).
    private(set) var isPreviewVisible = false
    /// True during the brief refinement recording cycle inside a preview.
    private var isRefinementRecording = false
    /// Dedicated recorder for refinement — avoids touching the main `recorder`.
    private var refinementRecorder: (any RecordingControlling)?
    /// Background task for stop-record → transcribe → inject flow.
    private var refinementRecordingTask: Task<Void, Never>?
    ...
    private struct PendingRetry {
        ...
        /// Capture-time skill/selection context so manual retry does not re-sample
        /// a drifted live selection.
        let preparedSkillContext: PreparedSkillContext
        let expiresAt: Date
    }
    ...
    func start(launchMode: AppLaunchMode = .normal) {
        do {
            ...
            if let launchBlocker = AppInstallLocation.launchBlocker() {
                ...
                notifier.notify(title: L10n.text("VibeCompose install required"), body: launchBlocker.message)
                ...
                if snapshotPrivacyMode.isEnabled {
                    ...
                    do {
                        quickAddHotkeyMonitor = try HotkeyMonitor(
                            keyCode: VibeComposeHotkeys.quickAddKeyCode,
                            modifiers: VibeComposeHotkeys.quickAddModifiers
                        ) { [weak self] in
                            Task { @MainActor in
                                self?.openQuickAdd()
                            }
    ...
    private func configureDictationHotkeyAtStartup() {
        ...
        case .fellBack(
            ...
            persistCoordinatorConfig(config, source: .external)
            applyActiveDictationHotkey(active)
            let detail = L10n.format(
                "%@ could not be registered. VibeCompose restored %@. Choose another shortcut in Settings. %@",
        ...
        case .unavailable(_, let reason):
            ...
            notifier.notify(
                title: L10n.text(
                    "VibeCompose shortcut unavailable"
                ),
    ...
    private func configureSkillSwitcherHotkeyAtStartup() {
        ...
        do {
            ...
            notifier.notify(
                title: L10n.text(
                    "VibeCompose Skill Switcher shortcut unavailable"
                ),
                body: L10n.format(
    ...
    private func configureResultPreviewHotkeyAtStartup() {
        ...
        do {
            ...
            notifier.notify(
                title: L10n.text(
                    "VibeCompose Result Preview shortcut unavailable"
                ),
                body: L10n.format(
                    "%@ could not be registered and was disabled. %@",
                    requested.displayName,
    ...
    func handleHotkeyPress() {
        logger.info(
            "Dictation trigger received shortcut=\(self.activeDictationHotkey.displayName, privacy: .public) state=\(String(describing: self.state), privacy: .public) isPreviewVisible=\(self.isPreviewVisible, privacy: .public)"
        )
        ...
        // D4: while the preview panel is open, hotkey drives voice refinement.
        if isPreviewVisible {
    ...
    func cancelCurrentSession(source: OverlayCancelSource? = nil) {
        ...
        processingTask = nil
        // Clear the preview flag before cancelling refinement so a mid-flight
        // refinement defer block does not call overlay.hide() again.
        isPreviewVisible = false
        cancelRefinementRecording()
    ...
    private func undoLastVerifiedInsertion(
        ...
        if outcome != .restored {
            notifier.notify(
                title: L10n.text("VibeCompose Safe Undo"),
        ...
        startRecordingTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                if let message = RuntimePreflight.summary(for: issues) {
                    ...
                    self.notifier.notify(title: L10n.text("VibeCompose setup required"), body: message)
                ...
                self.notifier.notify(title: "VibeCompose", body: error.localizedDescription)
                self.launchAppContext = nil
                self.recordProductMetric(
                    event: .dictationFailed,
                    provider: self.config.transcription.provider,
    ...
    private func stopRecording() {
        ...
        do {
            ...
            notifier.notify(title: "VibeCompose", body: error.localizedDescription)
            recordProductMetric(
                event: .dictationFailed,
                provider: config.transcription.provider,
                failureCategory: .recording
    ...
    private func retryPendingTranscription() {
        ...
        startProcessing(
            ...
            preparedSkillContext:
                pendingRetry.preparedSkillContext
        )
    }
    ...
    private func startProcessing(
        ...
        processingTask = Task { [weak self] in
            ...
            do {
                ...
                guard !Task.isCancelled else {
                    await MainActor.run {
                        self.clearStaleProcessingHUD(sessionID: sessionID)
                    }
                    return
                }
                self.logger.info(
                ...
                if route == .preview {
                    ...
                    let skillChoices = await MainActor.run {
                        self.previewSkillChoices(
                            targetContext: launchAppContext
                        )
                    }
                    ...
                    self.cancelRefinementRecording()
                    guard !Task.isCancelled else {
                        return
                    }
                ...
                do {
                    ...
                    let sessionStillActive = await MainActor.run {
                        self.shouldContinue(sessionID: sessionID)
                    }
                ...
                await MainActor.run {
                        ...
                        .completionNotificationEnabled
                    {
                        self.notifier.notify(
                            title: L10n.text(
                                "VibeCompose dictation complete"
                            ),
                ...
                // cancelCurrentSession normally hides the HUD; if the session was
                // invalidated without that path, do not leave "Processing" stuck.
                // Only hide when still in .processing — cancel already tore the
                // surface down and a second hide inflates call counts / races HUD.
                await MainActor.run {
    ...
    private func openSettings(
        ...
        if preferencesWindowController == nil {
            ...
            preferencesWindowController = PreferencesWindowController(
                ...
                onSave: { [weak self] newConfig in
                    guard let self else {
                        return .failure(
                            NSError(
                                domain: "VibeCompose.Preferences",
                                code: 2,
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text("VibeCompose settings are no longer available."),
                                ]
                            )
                        )
                    }
                ...
                onImportTerminologyDictionary: { [weak self] currentConfig, fileURL in
                    ...
                    do {
                        ...
                        try self.persistCoordinatorConfigThrowing(
                            updatedConfig,
                            source: .settings
                        )
                        self.refreshReadyState(
                            ...
                            NSError(
                                ...
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text(
                                        "VibeCompose settings are no longer available."
                                    ),
                                ]
                            )
                        )
                ...
                onExportSupportDiagnostics: { [weak self] destinationURL in
                    guard let self else {
                        return .failure(
                            NSError(
                                domain: "VibeCompose.Diagnostics",
                                code: 1,
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text(
                                        "VibeCompose settings are no longer available."
                ...
                onExportProductMetrics: { [weak self] destinationURL in
                    guard let self else {
                        return .failure(
                            NSError(
                                domain: "VibeCompose.ProductMetrics",
                                code: 1,
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text(
                                        "VibeCompose settings are no longer available."
                ...
                onDeleteTranscriptionRecord: { [weak self] id in
                    guard let self else {
                        return .failure(
                            NSError(
                                domain: "VibeCompose.History",
                                code: 1,
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text(
                                        "VibeCompose history is no longer available."
                                    ),
                                ]
                            )
                        )
            ...
            preferencesWindowController?.navigate(
                to: focusPane,
                skillLibrarySection: focusPane == .skills
                    ? skillLibraryInitialSection
                    : nil
            )
    ...
    private func openSkillSwitcher() {
        ...
        // Resolve app-scoped Skills against the host that still owns focus —
        // not against VibeCompose after the panel later activates us.
        ...
        terminologyQuickAddWindowController = TerminologyQuickAddWindowController(
            ...
            onSave: { [weak self] entry in
                guard let self else {
                    return .failure(
                        NSError(
                            domain: "VibeCompose.Terminology",
                            code: 2,
                            userInfo: [
                                NSLocalizedDescriptionKey: L10n.text(
                                    "VibeCompose terminology is no longer available."
                                ),
                            ]
                        )
                    )
        ...
        do {
            ...
            do {
                    ...
                    != newConfig.transcription.dictationHotkey
                {
                    ...
                    try persistCoordinatorConfigThrowing(
                        newConfig,
                        source: .settings
                    )
                }
    ...
    /// Persist coordinator memory + disk and optionally rebase an open Settings draft.
    private func persistCoordinatorConfig(
        _ newConfig: AppConfig,
        source: ConfigCommitSource
    ) {
        try? persistCoordinatorConfigThrowing(
            newConfig,
            source: source
    ...
    private func persistCoordinatorConfigThrowing(
        _ newConfig: AppConfig,
        source: ConfigCommitSource
    ) throws {
        if !snapshotPrivacyMode.isEnabled {
        ...
        config = newConfig
        if source == .external {
            preferencesWindowController?.applyLiveConfig(newConfig)
        }
    }
            ...
            onboardingWindowController = OnboardingWindowController(
                ...
                onRequestMicrophoneAccess: { [weak self] in
                    guard let self else {
                        return .failure(
                            NSError(
                                domain: "VibeCompose.Onboarding",
                                code: 1,
                                userInfo: [
                                    NSLocalizedDescriptionKey: L10n.text("VibeCompose setup is no longer available."),
                                ]
                            )
                        )
                    }
    ...
    private func scheduleSettingsSnapshotIfRequested() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { [weak self] in
            ...
            do {
                ...
                print("VibeCompose Settings self-capture failed: \(error.localizedDescription)")
            }
        }
    ...
    private func scheduleOnboardingSnapshotIfRequested() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { [weak self] in
            ...
            do {
                ...
                print("VibeCompose Onboarding self-capture failed: \(error.localizedDescription)")
            }
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
            NSApplication.shared.terminate(nil)
    ...
    private func scheduleProductSurfaceSnapshotIfRequested(mode: AppLaunchMode) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            do {
                ...
                print("VibeCompose product surface self-capture failed: \(error.localizedDescription)")
            }
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
            NSApplication.shared.terminate(nil)
    ...
    private func scheduleAccessibilityAuditIfRequested(
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            do {
                ...
                print(
                    "VibeCompose accessibility audit failed: \(error.localizedDescription)"
                )
            }
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
    ...
    func checkLaunchLoginState() {
        ...
        notifier.notify(title: L10n.text("VibeCompose setup required"), body: snapshot.detail)
    }
    ...
    private func showInstallRequiredAlert(message: String) {
        ...
        alert.messageText = L10n.text("Install VibeCompose to /Applications first")
        ...
        if let outputURL = AppLaunchMode.previewSnapshotOutputURL(
            ...
            DispatchQueue.main.asyncAfter(
                ...
                do {
                    ...
                    print(
                        "VibeCompose Preview self-capture failed: "
                            + error.localizedDescription
                    )
                }
            }
            ...
            DispatchQueue.main.asyncAfter(
                ...
                do {
                    ...
                    print(
                        "VibeCompose Preview accessibility audit failed: "
                            + error.localizedDescription
                    )
                }
            }
        ...
        if let feedbackDebugOutputURL {
            DispatchQueue.main.asyncAfter(
                ...
                do {
                    ...
                    print(
                        "VibeCompose feedback debug capture failed: "
                            + error.localizedDescription
                    )
                }
            }
        ...
        if let snapshotOutputURL {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [overlay] in
                do {
                    ...
                    print("VibeCompose HUD self-capture failed: \(error.localizedDescription)")
                }
            }
        }
        if let followupSnapshotOutputURL {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) { [overlay] in
                do {
                    ...
                    print(
                        "VibeCompose HUD follow-up self-capture failed: "
                            + error.localizedDescription
                    )
                }
            }
    ...
    /// Begin a dedicated refinement recording while the preview panel is open.
    /// Uses a fresh `AudioRecorder` so the main `recorder` / processing task
    /// are not disturbed.
    private func startRefinementRecording() {
        guard !isRefinementRecording, isPreviewVisible else { return }

        let sampleRate = config.transcription.sampleRateHz
        let rec = recorderFactory(sampleRate)
        // Claim the slot before the async mic open so a second hotkey
        // cannot spawn a parallel refinement recorder.
        isRefinementRecording = true
        refinementRecorder = rec

        refinementRecordingTask?.cancel()
        refinementRecordingTask = Task { @MainActor [weak self] in
            guard let self else { return }
            do {
                try await rec.startRecording()
            } catch {
                logger.error(
                    "Refinement recording failed to start: \(error.localizedDescription, privacy: .public)"
                )
                self.isRefinementRecording = false
                self.refinementRecorder = nil
                return
            }
            guard self.isPreviewVisible, self.refinementRecorder === rec else {
                // Preview closed or recording was cancelled while the mic opened.
                do { try rec.cancelRecording() } catch { /* best-effort */ }
                return
            }
            self.overlay.showRecording(elapsedText: "00:00")
            self.soundFeedback.play(
                .recordingStarted,
                enabled: self.config.transcription.feedbackSoundsEnabled
            )
        }
    }

    /// Stop the refinement recording, transcribe it (direct / polish-off), and
    /// inject a refinement prompt that pairs the spoken instruction with the
    /// previous preview result into the open panel.
    private func stopRefinementRecording() {
        guard isRefinementRecording, let rec = refinementRecorder else { return }

        isRefinementRecording = false
        refinementRecorder = nil

        do {
            let audio = try rec.stopRecording()
            soundFeedback.play(
                .recordingStopped,
                enabled: config.transcription.feedbackSoundsEnabled
            )
            overlay.showProcessing()

            // Transcription only: force Direct skill and disable polish so the
            // pipeline returns the spoken refinement instruction raw.
            var transcriptOnlyConfig = config.transcription
            transcriptOnlyConfig.skills = SkillsConfig()
            transcriptOnlyConfig.textPolish.mode = .disabled

            let previousResult = lastDictationSession?.lastResultText ?? ""
            let authManager = self.authManager
            let providerCapabilityPolicy = self.providerCapabilityPolicy
            let recoveryCredentialStore = self.recoveryCredentialStore
            let pipelineFactory = self.pipelineFactory

            refinementRecordingTask = Task { @MainActor [weak self] in
                guard let self else { return }
                defer {
                    // Only hide the HUD when preview is still up — cancel may
                    // already have torn the surface down.
                ...
                do {
                    ...
                    let prepared = try await pipeline.prepare(audio: audio)
                    // Prefer raw ASR; fall back to finalText if raw is empty.
                    let transcript = prepared.rawText
                        .trimmingCharacters(in: .whitespacesAndNewlines)
                    let instruction = transcript.isEmpty
                        ? prepared.finalText.trimmingCharacters(
                            in: .whitespacesAndNewlines
                        )
                        : transcript
                    guard !instruction.isEmpty else {
                        logger.info(
                            "Refinement transcript was empty; skipping inject"
                        )
                        return
                    }
                    guard self.isPreviewVisible else {
                        logger.info(
                            "Preview closed before refinement inject; discarding"
                        )
                        return
                    }
                    let refinementSource = Self.makeVoiceRefinementSource(
                        instruction: instruction,
                        previousResult: previousResult
                    )
                    logger.info(
                        "Refinement transcript ready characters=\(instruction.count, privacy: .public); injecting into preview"
                    )
                    await self.previewPresenter.injectVoiceRefinement(
                        refinementSource
                    )
                } catch {
                    logger.error(
                        "Refinement transcription failed: \(error.localizedDescription, privacy: .public)"
                    )
                }
            }
        } catch {
            logger.error(
                "Refinement recording stop failed: \(error.localizedDescription, privacy: .public)"
            )
            isRefinementRecording = false
            refinementRecorder = nil
            overlay.hide()
    ...
    /// Compose the regenerate input for a voice refinement: the spoken
    /// instruction is primary, and the previous result is attached so the
    /// active skill can revise it rather than starting from scratch.
    /// Internal for unit tests (`@testable import`).
    static func makeVoiceRefinementSource(
        instruction: String,
        previousResult: String
    ) -> String {
        let trimmedPrevious = previousResult.trimmingCharacters(
            in: .whitespacesAndNewlines
        )
        guard !trimmedPrevious.isEmpty else {
            return instruction
        }
        return """
        Refine the previous result using this instruction. Output only the revised text.
    ...
    private func cancelRefinementRecording() {
        refinementRecordingTask?.cancel()
        refinementRecordingTask = nil
        if isRefinementRecording {
            do {
                try refinementRecorder?.cancelRecording()
            } catch {
                logger.error(
                    "Cancelling refinement recording failed: \(error.localizedDescription, privacy: .public)"
                )
            }
        }
        isRefinementRecording = false
    ...
    private func storePendingRetry(
        ...
        // Keep the capture-time skill/selection prompt on the retry config so
        // polish and routing see the same context as the original attempt.
        let retryTranscriptionConfig = transcriptionConfig
    ...
    func deleteAllUserData() -> Result<AppConfig, any Error> {
        ...
        do {
            ...
            // Memory already updated inside persist; re-assert for clarity after
            // replace closure and still push live into any open Settings draft
            // after wipe so the shell reseeds to defaults.
            config = freshConfig
            preferencesWindowController?.applyLiveConfig(
                freshConfig,
                forceReplace: true
            )
            ...
            refreshReadyState(
                detailOverride: L10n.text("All VibeCompose data was deleted. Connect ChatGPT again to continue."),
                state: .setupRequired
            )
        } catch {
            if firstError == nil {
    ...
    /// Tear down a processing presentation when the session is no longer active
    /// and nothing else has already transitioned the HUD away from "Processing".
    private func clearStaleProcessingHUD(sessionID: UUID) {
        guard activeSessionID == sessionID || activeSessionID == nil else {
            return
        }
    ...
    private func previewSkillChoices(
        targetContext: LaunchAppContext? = nil
    ) -> [SkillMenuEntry] {
        refreshSkillMenu(launchAppContext: targetContext)
        guard let snapshot = latestSkillMenuSnapshot else {
            return []
    ...
    private func regeneratePreviewRequest(
        ...
        do {
            ...
            // Prefer the user-edited Preview source when regenerating; keep
            // originalTranscript on the session for history/source labels.
            let inputText = editedSourceInput ?? session.originalTranscript
            let execution = try await SkillTestEngine().execute(
                SkillTestRunRequest(
                    plan: plan,
                    inputText: inputText,
    ...
    private func reopenLastResultPreview() {
        ...
        // Capture before dismissing or presenting any VibeCompose surface.
        // Every branch in this Preview session stays bound to this page/window.
        let targetContext = launchAppContextProvider()
    ...
    private func beginSkillChangeFromPreview(
        editedSource: String,
        targetContext: LaunchAppContext?
    ) {
        ...
        previewPresenter.dismiss()
        openSkillSwitcherForReprocess(
            sourceOverride: editedSource,
            targetContext: targetContext
        )
    }

    private func openSkillSwitcherForReprocess(
        sourceOverride: String?,
        targetContext: LaunchAppContext?
    ) {
        refreshSkillMenu(launchAppContext: targetContext)
        ...
        skillSwitcherWindowController =
            SkillSwitcherWindowController(
                ...
                onDismiss: { [weak self] in
                    ...
                    // Keep the original focused target across the whole flow.
                    if !commit.didCommit {
                        self.reopenPreviewAfterSkillChangeAbort(
                            targetContext: targetContext
                        )
    ...
    private func reprocessLastSession(
        ...
        // Prefer user-edited Preview source for Skill input; keep
        // originalTranscript on the session for history/source labels.
        var editedSourceInput: String?
        if let sourceOverride {
            let trimmed = sourceOverride.trimmingCharacters(
        ...
        processingTask = Task { [weak self] in
            ...
            do {
                ...
                let inputText =
                    editedSourceInput ?? session.originalTranscript
                let execution = try await SkillTestEngine().execute(
                    SkillTestRunRequest(
                        plan: plan,
                        inputText: inputText,
    ...
    private func presentCachedPreview(
        ...
        case let .replaceSelection(text),
            ...
            if allowsPaste {
                do {
                    ...
                    let expectedSelection: SelectionContextSnapshot? = {
                        if case .replaceSelection = decision {
                    ...
                    let outcome = try await injector.inject(
                        ...
                        launchAppContext: targetContext,
                        automaticPasteAllowed: true,
                        automaticPasteFallbackReason:
                            .deliveryRequiresManualPaste,
                        expectedSelectionContext: expectedSelection
                    )
                    await MainActor.run {
                        guard self.shouldContinue(sessionID: sessionID) else {
                            return
