...
struct ProviderRequestFailure: Error, LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .challenge:
            if route == .textPolish {
                return L10n.format(
                    "%@ hit an upstream challenge. VibeCompose kept the normalized transcript.",
                    name
                )
            }
            return L10n.format(
        ...
        case .rateLimited:
            ...
            if route == .textPolish {
                return L10n.format(
                    "%@ is rate limited. VibeCompose kept the normalized transcript; try again in about %ld seconds.",
                    name,
                    seconds
                )
            }
        ...
        case .contractChanged:
            return L10n.format(
                "%@ contract changed. Update VibeCompose before retrying this provider.",
                name
            )
        case .serviceUnavailable:
            if route == .textPolish {
                return L10n.format(
                    "%@ is temporarily unavailable after %ld attempts. VibeCompose kept the normalized transcript.",
                    name,
                    attempts
                )
            }
        ...
        case .network:
            if route == .textPolish {
                return L10n.format(
                    "%@ could not reach the network after %ld attempts. VibeCompose kept the normalized transcript.",
                    name,
                    attempts
                )
            }
