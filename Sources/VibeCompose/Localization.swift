...
extension Notification.Name {
    static let vibeComposeLanguageDidChange = Notification.Name(
        "VibeCompose.LanguageDidChange"
    )
    static let vibeComposeNavigateSettingsPane = Notification.Name(
        "VibeCompose.NavigateSettingsPane"
    )
    /// Posted when AppCoordinator commits config from a non-Settings source so
    /// an open Preferences draft can rebase without last-write-wins clobber.
    static let vibeComposeLiveConfigDidChange = Notification.Name(
        "VibeCompose.LiveConfigDidChange"
    )
}
...
enum L10n {
    ...
    static func setLanguage(
        ...
        NotificationCenter.default.post(
            name: .vibeComposeLanguageDidChange,
            object: language
        )
    }
