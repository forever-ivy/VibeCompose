    ...
    Equatable
{
    ...
    var errorDescription: String? {
        ...
        case .symbolicLink(let path):
            return L10n.format(
                "VibeCompose blocked a symbolic link in the Skill package: %@",
                path
            )
        case .pathTraversal(let path):
            return L10n.format(
                "VibeCompose blocked an unsafe Skill package path: %@",
                path
            )
        ...
        case .incompatibleVersion(let version):
            return L10n.format(
                "This Skill requires VibeCompose %@ or later.",
                version
            )
    ...
    Identifiable
{
    ...
    init(
        ...
        isActive: Bool,
        format: SkillPackageFormat = .legacyVibeComposeV1,
...
/// Explicit compatibility boundary for the original VibeCompose v1 package.
...
struct LegacyVibeComposeV1Adapter:
    Sendable
{
    func normalize(
        definition: SkillDefinition,
    ...
    @unchecked Sendable
{
    ...
    func inspect(
        ...
        return LegacyVibeComposeV1Adapter().normalize(
            definition: definition,
            packageURL: packageURL,
            relativeFiles:
    ...
    func install(
        ...
        let isLegacyTransport =
            packageExtension == "vibecomposeskill"
            || packageExtension
                == LegacyProductIdentity.skillPackageExtension
        ...
        guard isLegacyTransport || isStandardDirectory else {
            throw CommunitySkillPackageError
                .invalidPackage(
                    "choose an Agent Skills directory or a .vibecomposeskill directory"
                )
        }
        let inspected =
            try inspect(
    ...
    func installRegistryPackage(
        ...
        let identity = InstalledSkillIdentity(
            id: StableIdentifier.uuid(
                namespace: "VibeCompose.RegistrySkillInstallation",
                components: [
    ...
    private func isSupportedArchive(
        ...
        guard pathExtension == "zip"
                || pathExtension == "vibecomposeskill"
                || pathExtension
                    == LegacyProductIdentity.skillPackageExtension
        else {
            return false
    ...
    private func extractArchive(
        ...
        let container = fileManager.temporaryDirectory
            .appendingPathComponent(
                "VibeCompose-SkillArchive-\(UUID().uuidString)",
                isDirectory: true
            )
    ...
    private func runArchiveTool(
        ...
        let captureRoot = fileManager.temporaryDirectory
            .appendingPathComponent(
                "VibeCompose-ArchiveTool-\(UUID().uuidString)",
                isDirectory: true
            )
    ...
    private func validatedFiles(
        ...
        guard
            ...
            throw CommunitySkillPackageError
                .invalidPackage(
                    "choose a .vibecomposeskill directory"
                )
        }
    ...
    private func validateManifest(
        ...
        guard
            ...
            !manifest.id.hasPrefix(
                "app.vibecompose.skill."
            )
        else {
            throw CommunitySkillPackageError
                .invalidManifest(
                    "use a reverse-domain ID that does not reserve VibeCompose's built-in namespace"
                )
        }
        guard
            SkillDefinition
            ...
            .map { entry in
                ...
                entry.id =
                    StableIdentifier.uuid(
                        namespace:
                            "VibeCompose.CommunitySkillTerminology",
                        components: [
                            skillID,
                            entry.type
                                .rawValue,
    ...
    private func contentHash(
        ...
        hasher.update(
            data: Data(
                "VibeComposeCommunitySkillV1\u{0}"
                    .utf8
            )
        )
