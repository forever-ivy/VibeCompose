    ...
    Identifiable
{
    ...
    /// English source strings for the Skill Library summary / use-case copy.
    /// Built-in packages declare them (`metadata.summary` / `metadata.use-case`);
    /// they double as L10n keys so translations resolve at display time.
    let summary: String?
    let useCase: String?
...
extension SkillDefinition {
    /// Summary and use-case copy is declared by the Skill package
    /// (`metadata.summary` / `metadata.use-case`). The English strings double
    /// as localization keys, so built-in Skills keep their translations.
    var localizedSummary: String {
        guard let summary, !summary.isEmpty else {
            return promptInstruction
        }
        return L10n.text(summary)
    }

    var localizedUseCase: String {
        guard let useCase, !useCase.isEmpty else {
            return localizedSummary
        }
        return L10n.text(useCase)
    }
}
    ...
    Equatable
{
    static let directSkillID =
        "app.vibecompose.skill.direct"
    static let replySkillID =
        "app.vibecompose.skill.reply"
    static let emailSkillID =
        "app.vibecompose.skill.email"
    static let codePromptSkillID =
        "app.vibecompose.skill.code-prompt"
    static let translateSkillID =
        "app.vibecompose.skill.translate"
    static let contextRewriteSkillID =
        "app.vibecompose.skill.context-rewrite"
    static let bugReportSkillID =
        "app.vibecompose.skill.bug-report"
    static let commitMessageSkillID =
        "app.vibecompose.skill.commit-message"
    static let meetingActionItemsSkillID =
        "app.vibecompose.skill.meeting-action-items"
    static let productBriefSkillID =
        "app.vibecompose.skill.product-brief"
    static let customerSupportReplySkillID =
        "app.vibecompose.skill.customer-support-reply"

    // New skills
    static let contextSummarizeSkillID =
        "app.vibecompose.skill.context-summarize"
    static let standupUpdateSkillID =
        "app.vibecompose.skill.standup-update"
    static let changelogEntrySkillID =
        "app.vibecompose.skill.changelog-entry"
    static let betterQuestionSkillID =
        "app.vibecompose.skill.better-question"
    static let codeReviewCommentSkillID =
        "app.vibecompose.skill.code-review-comment"
    static let socialPostSkillID =
        "app.vibecompose.skill.social-post"
    static let frontendPromptSkillID =
        "app.vibecompose.skill.frontend-prompt"
    static let clipboardRewriteSkillID =
        "app.vibecompose.skill.clipboard-rewrite"
    static let paragraphPolishSkillID =
        "app.vibecompose.skill.paragraph-polish"
    static let incidentReportSkillID =
        "app.vibecompose.skill.incident-report"

    /// Retired built-in Skill IDs mapped to the Skill that absorbed them.
    /// `agent-plan` (Backend Prompt) merged into Code Prompt; `context-reply`
    /// merged into Reply, which now accepts an optional selection. References
    /// in persisted configuration are canonicalized on load — see
    /// `LegacyProductIdentity.canonicalizingBuiltInSkillReferences`.
    static let retiredSkillIDAliases: [String: String] = [
        "app.vibecompose.skill.agent-plan": codePromptSkillID,
        "app.vibecompose.skill.context-reply": replySkillID,
    ]

    /// Built-in Skills are declarative Agent Skills packages under
    /// `Resources/BuiltInSkills/<portable-name>/`. See `BuiltInSkillCatalog`.
    static let builtIn = SkillRegistry(
        definitions: BuiltInSkillCatalog.loadDefinitions()
    )
    ...
    Equatable
{
    ...
    var errorDescription: String? {
        ...
        case .invalidSkillIdentifier:
            return L10n.text(
                "Choose an installed VibeCompose Skill."
            )
        }
    }
}
    ...
    Identifiable
{
    ...
    private static func stableInstallationID(
        ...
        StableIdentifier.uuid(
            namespace:
                "VibeCompose.AppSkillRule.Installation",
            components: [skillID]
        )
    }
    ...
    Equatable
{
    ...
    init(
        ...
        profile: VibeComposeSkillProfile? = nil,
    ...
    Equatable
{
    ...
    var focusedParagraph: String?

    init(
        styleCapsule: String? = nil,
        selection: String? = nil,
        clipboard: String? = nil,
        focusedParagraph: String? = nil
    ) {
...
struct SkillPromptCompiler: Sendable {
    ...
    func compile(
        ...
        var sections: [String] = [
            Self.systemMarker,
            """
            You are VibeCompose's post-ASR transformation engine for macOS dictation.
        ...
        if
            plan.skill
                .allCapabilities
                .contains(.focusedParagraph),
            let paragraph =
                normalizedOptionalText(
                    context.focusedParagraph,
                    maximumCharacters:
                        ContextConfig
                            .maximumSelectionCharacterLimit
                )
