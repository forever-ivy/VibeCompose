...
enum FocusedElementInspector {
    ...
    static func captureSelectionContext(
        ...
        // front VibeCompose do not make selection look "gone". Fall back to the
    ...
    private static func selectedTextRange(on element: AXUIElement) -> CFRange? {
        ...
        let axValue = unsafeDowncast(rawRange, to: AXValue.self)
    ...
    // MARK: - Focused Paragraph Capture

    /// Captures the paragraph surrounding the insertion point in the focused
    /// editable element. A paragraph is the run of text between two newlines
    /// (or the start/end of the document). Returns `nil` when Accessibility is
    /// unavailable, the focused element has no readable value, or the cursor
    /// position cannot be determined.
    @MainActor
    static func captureFocusedParagraph(
        in launchAppContext: LaunchAppContext?,
        maximumCharacters: Int
    ) -> String? {
        guard
        ...
        // Read the insertion-point position via AXSelectedTextRange.
        ...
        // Use the start of the selection (= cursor) as the anchor.
        let cursorIndex = max(0, cfRange.location)
        let nsValue = value as NSString
        guard cursorIndex <= nsValue.length else { return nil }

        // Find the paragraph boundaries around the cursor by scanning
        // backwards for the preceding newline and forwards for the next.
        let startIndex: Int = {
            if cursorIndex == 0 { return 0 }
            let range = NSRange(location: 0, length: cursorIndex)
            let found = nsValue.rangeOfCharacter(
                from: .newlines,
                options: .backwards,
                range: range
            )
        ...
        let endIndex: Int = {
            if cursorIndex >= nsValue.length { return nsValue.length }
            let range = NSRange(
                location: cursorIndex,
                length: nsValue.length - cursorIndex
            )
            let found = nsValue.rangeOfCharacter(
                from: .newlines,
                options: [],
        ...
        let paragraph = nsValue.substring(
            with: NSRange(
                location: startIndex,
        ...
        let bounded = boundedSelectionMaximum(maximumCharacters)
        guard paragraph.count <= bounded else { return nil }
        return paragraph
    }
}
