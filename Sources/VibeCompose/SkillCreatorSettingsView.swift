...
struct SkillCreatorSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: VibeComposeMetrics.space12) {
            HStack(alignment: .firstTextBaseline, spacing: VibeComposeMetrics.space12) {
            ...
            if editorMode == .simple {
                ...
                TextEditor(text: $draft.instructions)
                    ...
                    .font(VibeComposeTypography.mono())
                    .frame(minHeight: 110, idealHeight: 140, maxHeight: 220)
                    .scrollContentBackground(.hidden)
                    .padding(VibeComposeMetrics.space8)
                    .background(
                        Color(nsColor: .textBackgroundColor),
                        in: RoundedRectangle(cornerRadius: VibeComposeMetrics.radiusS, style: .continuous)
                    )
                    .overlay {
                        RoundedRectangle(cornerRadius: VibeComposeMetrics.radiusS, style: .continuous)
            ...
            ViewThatFits(in: .horizontal) {
                HStack(spacing: VibeComposeMetrics.space14) { profileControls }
                VStack(alignment: .leading, spacing: VibeComposeMetrics.space10) { profileControls }
            }
            ...
            if let validationReport {
                if validationReport.isValid {
                    InlineStatus(
                        text: L10n.text("The draft passes standard format and VibeCompose runtime validation."),
            ...
            ViewThatFits(in: .horizontal) {
                HStack(spacing: VibeComposeMetrics.space10) { actionControls }
                VStack(alignment: .leading, spacing: VibeComposeMetrics.space8) { actionControls }
            }
    ...
    private var creatorModeControl: some View {
        HStack(spacing: VibeComposeMetrics.space4) {
                    ...
                    Text(L10n.text(mode.rawValue))
                        ...
                        .background(
                            editorMode == mode
                                ...
                                : Color.clear,
                            in: RoundedRectangle(cornerRadius: VibeComposeMetrics.radiusXS, style: .continuous)
                        )
                }
                .buttonStyle(.plain)
            }
        }
        .padding(VibeComposeMetrics.space4)
        .frame(maxWidth: 300)
        .background(
                ...
                if let testBenchReport {
                    ...
                    DisclosureGroup(
                        ...
                        Text(testBenchReport.prompt)
                            .font(VibeComposeTypography.mono(9))
                            .textSelection(.enabled)
    ...
    private var advancedResources: some View {
        VStack(alignment: .leading, spacing: VibeComposeMetrics.space10) {
            ViewThatFits(in: .horizontal) {
                HStack(spacing: VibeComposeMetrics.space8) { resourceButtons }
                VStack(alignment: .leading, spacing: VibeComposeMetrics.space8) { resourceButtons }
            }
