...
enum OpenAICompatibleCredentialStoreError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unhandledStatus(let status):
            return L10n.format(
                "VibeCompose could not access the OpenAI-Compatible API key in Keychain (OSStatus %d).",
                status
            )
        }
    }
    ...
    OpenAICompatibleCredentialPersisting
{
    ...
    func loadAPIKey() throws -> String? {
        ...
        do {
            ...
            return apiKey
        } catch OpenAICompatibleCredentialStoreError.unhandledStatus(
            let status
        ) where LegacyProductIdentity.isUnavailableLegacyKeychainStatus(
            status
        ) {
            // The new bundle identity may not be able to read the former
            // Keychain row. The UI can recover by asking the user to sign in.
            return nil
        }
    }
    ...
    func deleteAPIKey() throws {
        try deleteAPIKey(
            service: service,
            toleratingLegacyAccessFailure: false
        )
