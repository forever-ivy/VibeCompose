...
final class SonnerToastCenter: ObservableObject {
    ...
    /// Honor system Reduce Motion (and visual-acceptance override) at the
    /// source of truth so Host transition style and center animation agree.
    private func animateToastChange(_ updates: () -> Void) {
        let reduceMotion = AccessibilityDisplayOptions.current.reduceMotion
        if reduceMotion {
            var transaction = Transaction()
            transaction.disablesAnimations = true
            withTransaction(transaction, updates)
        } else {
            withAnimation(.spring(response: 0.32, dampingFraction: 0.86), updates)
        }
    }
}
        ...
        VStack {
            if let item = center.current {
                SonnerToastBanner(item: item)
                    ...
                    .padding(.top, VibeComposeMetrics.space12)
                    .zIndex(1)
            }
            Spacer(minLength: 0)
        }
