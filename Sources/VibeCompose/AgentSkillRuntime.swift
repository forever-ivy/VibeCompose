...
struct VibeComposeSkillProfile:
    Codable,
    Sendable,
    Equatable
{
    ...
    static let safeDefault = VibeComposeSkillProfile(
    ...
    static func normalized(
        ...
        return InstalledSkillIdentity(
            id: StableIdentifier.uuid(
                namespace: "VibeCompose.InstalledSkillIdentity",
                components: [
                    sourceID,
                    packageID,
                    definition.id,
    ...
    Equatable
{
    ...
    let runtimeStatus: VibeComposeRuntimeStatus
    ...
    Equatable
{
    ...
    var errorDescription: String? {
        ...
        case .invalidProfile(let detail):
            return L10n.format("vibecompose.yaml is invalid: %@", detail)
        ...
        case .incompatible(let detail):
            return L10n.format("The Skill is not compatible with VibeCompose: %@", detail)
        }
    }
}
...
struct VibeComposeProfileLoader:
    Sendable
{
    func load(from url: URL?) throws -> VibeComposeSkillProfile {
        ...
        return VibeComposeSkillProfile(
            contextRequest: ContextRequest(
            ...
            validators: SkillValidatorPolicy(
                ...
                requiredSectionAlternatives: document.list("validators.requiredSections").map {
    ...
    @unchecked Sendable
{
    ...
    private func enumerateFiles(rootURL: URL) throws -> [FileEntry] {
        ...
        // Do not use `.skipsHiddenFiles`: macOS UF_HIDDEN (often re-applied by
        // Desktop/iCloud File Provider after packaging) would hide legitimate
        // SKILL.md / vibecompose.yaml resources and make built-in Skills unloadable.
        // Dot-prefixed junk is filtered below by path instead.
    ...
    private func kind(for path: String, prefix: Data.SubSequence) -> SkillResourceKind {
        ...
        if path == "vibecompose.yaml" { return .profile }
    ...
    Sendable
{
    func analyze(
        ...
        if hasAllowedTools {
            issues.append("allowed-tools requests tools that VibeCompose does not expose")
        }
        ...
        } else if package.resources.contains(where: { $0.relativePath == "vibecompose.yaml" }) {
            level = .vibeComposeEnhanced
        } else {
            level = .portable
        }
    ...
    Sendable
{
    func normalize(
        ...
        let profileURL = package.resources.contains {
            $0.relativePath == "vibecompose.yaml"
        } ? package.rootURL.appendingPathComponent("vibecompose.yaml") : nil
        let profile = try VibeComposeProfileLoader().load(from: profileURL)
        let compatibility = SkillCompatibilityAnalyzer().analyze(
            package: package,
            profile: profile
        )
        let digestPrefix = String(package.contentSHA256.prefix(12))
        let metadataID = package.metadata.metadata["vibecompose-id"]?.lowercased()
...
/// Loads the app-owned built-in Skill packages shipped under
/// `Resources/BuiltInSkills/<portable-name>/` (Agent Skills standard + optional
/// `vibecompose.yaml`). Reserved `app.vibecompose.skill.*` IDs are allowed only
/// for this catalog path.
enum BuiltInSkillCatalog {
    static let expectedCount = 21

    /// Stable menu and resolver order. Directory names match `SKILL.md` `name`.
    /// Retired packages (`backend-prompt` → code-prompt, `context-reply` →
    /// reply) are aliased in `SkillRegistry.retiredSkillIDAliases`.
    static let orderedPortableNames: [String] = [
        // Core dictation
        "direct",
        "reply",
        "email",
    ...
    static func loadDefinitions(
        ...
        guard let root = locateRoot(fileManager: fileManager) else {
            assertionFailure(
                "BuiltInSkills directory is missing from the app bundle and source tree."
            )
            return []
        }
        ...
        for portableName in orderedPortableNames {
            ...
            guard fileManager.fileExists(atPath: packageURL.path) else {
                assertionFailure(
                    "Missing built-in Skill package: \(portableName)"
                )
                continue
            }
            do {
                let package = try loader.load(from: packageURL)
                let definition = try definition(from: package)
                guard seenIDs.insert(definition.id).inserted else {
                    assertionFailure(
                        "Duplicate built-in Skill ID: \(definition.id)"
                    )
                    continue
                }
                ...
                assertionFailure(
                    "Failed to load built-in Skill \(portableName): \(error)"
                )
            }
        }
        ...
        assert(
            ...
            "Expected \(expectedCount) built-in Skills, loaded \(definitions.count)"
        )
        return definitions
    }

    static func locateRoot(
        fileManager: FileManager = .default
    ) -> URL? {
        if let resourceRoot = Bundle.main.resourceURL?
            .appendingPathComponent("BuiltInSkills", isDirectory: true),
            fileManager.fileExists(atPath: resourceRoot.path)
        {
            return resourceRoot
        }

        // Packaged apps copy Resources flat; also accept a nested path used by
        // some bundlers.
        if let nested = Bundle.main.url(
            forResource: "BuiltInSkills",
            withExtension: nil
        ),
            fileManager.fileExists(atPath: nested.path)
        {
            return nested
        }
    ...
    private static func definition(
        from package: AgentSkillPackage
    ) throws -> SkillDefinition {
        let profileURL = package.resources.contains {
            $0.relativePath == "vibecompose.yaml"
        }
            ? package.rootURL.appendingPathComponent("vibecompose.yaml")
            : nil
        let profile = try VibeComposeProfileLoader().load(from: profileURL)

        guard
            let rawID = package.metadata.metadata["vibecompose-id"]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased(),
            SkillDefinition.isValidIdentifier(rawID),
            rawID.hasPrefix("app.vibecompose.skill.")
        else {
            throw AgentSkillPackageError.incompatible(
                "built-in Skill is missing a reserved vibecompose-id"
            )
        }
        ...
        let displayName = package.metadata.metadata["display-name"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        let author = package.metadata.metadata["author"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let legacyMode = package.metadata.metadata["legacy-mode"]
            .flatMap(DictationMode.init(rawValue:))
        let summary = package.metadata.metadata["summary"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let useCase = package.metadata.metadata["use-case"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let terminology = try loadTerminology(
            package: package,
            skillID: rawID
        )
        ...
        let optionalCapabilities = capabilities(
            profile.contextRequest.optional,
            includeVoice: false
        )
        ...
        return SkillDefinition(
            ...
            version: version,
            name: name,
            author: author?.isEmpty == false ? author! : "VibeCompose",
            requiredCapabilities: requiredCapabilities,
            optionalCapabilities: optionalCapabilities,
            terminologyEntries: terminology,
            promptInstruction: package.instructions,
            ...
            useCase: useCase?.isEmpty == false ? useCase : nil
        )
    }
    ...
    private static func loadTerminology(
        ...
        guard let descriptor else { return [] }
        let url = package.rootURL
            .appendingPathComponent(descriptor.relativePath)
        let result = try TerminologyTextImporter().importEntries(
            from: Data(contentsOf: url, options: [.mappedIfSafe]),
            sourceName: descriptor.relativePath
        )
