...
final class StatusMenuController: NSObject, StatusMenuUpdating, NSMenuDelegate {
    ...
    private var currentDetail: String = L10n.text("Ready")
    ...
    func update(state: StatusMenuVisualState, detail: String) {
        ...
        if let button = statusItem.button {
            button.title = ""
            button.image = VibeComposeStatusIconRenderer.image(for: state)
            button.imagePosition = .imageOnly
            button.setAccessibilityLabel(L10n.format("VibeCompose %@", state.stateDescription))
            button.toolTip = L10n.format("VibeCompose: %@", state.stateDescription)
        }
    ...
    func reloadLocalization() {
        ...
        // When idle/ready, always rebuild the hotkey template so language
        // changes and hotkey renames stay correct. Mid-session (recording/
        // processing/error) keep the live detail via L10n.text so we do not
        // force a false "Ready" status.
        if currentState == .ready {
            detailItem.title = L10n.format(
                "Ready. Press %@ to dictate",
                dictationHotkey.displayName
            )
            currentDetail = detailItem.title
        } else {
            detailItem.title = L10n.text(currentDetail)
        }
    ...
    private func configureMenu() {
        ...
        let quitItem = NSMenuItem(
            title: L10n.text("Quit VibeCompose"),
    ...
    private func addSkillSection(
        ...
        for entry in entries {
            ...
            item.image = entry.presentation.menuImage(pointSize: 13)
