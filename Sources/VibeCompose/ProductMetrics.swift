...
enum ProductMetricsStorageError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unsafeStorage(let path):
            return "VibeCompose refused unsafe product metrics storage at \(path)."
        }
    }
}
...
enum ProductMetricsExportError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .destinationIsDirectory(let name):
            return L10n.format(
                "VibeCompose cannot replace the folder named %@ with a product metrics report.",
                name
            )
        }
    }
...
struct ProductMetricsExporter {
    ...
    static func suggestedFileName(now: Date = Date()) -> String {
        ...
        return "VibeCompose-Product-Metrics-\(formatter.string(from: now)).json"
    }
    ...
    func export(
        ...
        let stagedURL = destinationURL
            ...
            .appendingPathComponent(
                ".vibecompose-product-metrics-\(UUID().uuidString).json"
            )
        defer {
