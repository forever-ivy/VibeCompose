...
enum EndpointPolicyError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .disallowedManagedEndpoint:
            return L10n.text("VibeCompose blocked a non-approved endpoint for the managed ChatGPT session.")
        }
    }
}
...
enum ManagedEndpointPolicy {
    ...
    private static func makeURL(_ value: String) -> URL {
        guard let url = URL(string: value) else {
            preconditionFailure("Invalid built-in VibeCompose endpoint: \(value)")
        }
        return url
    }
}
