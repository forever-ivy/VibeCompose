...
enum BenchmarkError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingAudioFiles:
            return "Set VIBECOMPOSE_BENCHMARK_AUDIO_FILES to one or more local audio file paths."
...
struct BenchmarkRunner {
    ...
    func run() async throws {
        ...
        let runs = Int(environment["VIBECOMPOSE_BENCHMARK_RUNS"] ?? "") ?? 5
    ...
    private func benchmarkAudioFiles() -> [URL] {
        let rawValue = environment["VIBECOMPOSE_BENCHMARK_AUDIO_FILES"] ?? ""
        return rawValue
            .split(separator: ",")
