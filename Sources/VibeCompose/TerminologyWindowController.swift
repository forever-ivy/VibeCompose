...
/// Top-level tabs on the terminology pane: the personal dictionary manager and
/// the one-tap domain terminology packs. Both operate on the same `config`
/// snapshot and share the pane's `persist()` path, so there is a single source
/// of truth for enabled state and saving.
///
/// The Packs tab is implemented but hidden from the product surface for now
/// (`packsUIEnabled == false`). Flip the flag when shipping domain packs.
private enum TerminologyTab: String, CaseIterable, Identifiable {
    case entries = "My Terms"
    case packs = "Packs"

    /// Product gate for the Packs tab. Keep `false` until domain packs ship.
    static let packsUIEnabled = false

    var id: String { rawValue }

    var symbolName: String {
        switch self {
        case .entries: return "character.book.closed"
        case .packs: return "square.stack.3d.up"
        }
    }

    /// Tabs currently shown in the UI. When only one tab is visible the bar
    /// itself is suppressed so the page does not look like a half-finished shell.
    static var visibleCases: [TerminologyTab] {
        allCases.filter { tab in
            switch tab {
            case .entries: return true
            case .packs: return packsUIEnabled
            }
        }
    }
}
...
final class TerminologyWindowController: NSWindowController {
    init(
        ...
        window.title = L10n.text("VibeCompose Terminology")
        window.isReleasedWhenClosed = false
        window.isRestorable = false
        window.identifier = NSUserInterfaceItemIdentifier("VibeCompose.TerminologyWindow")
        window.collectionBehavior.remove(.fullScreenPrimary)
        window.standardWindowButton(.zoomButton)?.isEnabled = false
        window.minSize = NSSize(width: 760, height: 500)
        window.tabbingMode = .disallowed
        let restored = window.setFrameUsingName("VibeCompose.TerminologyWindow")
        window.setFrameAutosaveName("VibeCompose.TerminologyWindow")
        if !restored {
            window.center()
        }
...
struct TerminologyManagerView: View {
    ...
    @State private var activeTab: TerminologyTab = .entries
    ...
    var body: some View {
        VStack(spacing: 0) {
            if isEmbedded {
                VibeComposePaneHeader(title: L10n.text("Terminology")) {
            ...
            // Hide the tab bar while Packs is product-gated; reappears when
            // more than one tab is visible.
            if TerminologyTab.visibleCases.count > 1 {
                tabBar
            }
            ...
            case .entries:
                entriesTab
            case .packs:
                if TerminologyTab.packsUIEnabled {
                    packsTab
                } else {
                    entriesTab
                }
        ...
        .onAppear {
            if !TerminologyTab.packsUIEnabled, activeTab == .packs {
                activeTab = .entries
            }
        }
    ...
    private var tabBar: some View {
        HStack(spacing: VibeComposeMetrics.space6) {
            ForEach(TerminologyTab.visibleCases) { tab in
                tabButton(tab)
            }
            Spacer(minLength: 0)
        }
        .padding(.horizontal, VibeComposeMetrics.space20)
        .padding(.top, isEmbedded ? 0 : VibeComposeMetrics.space14)
        .padding(.bottom, VibeComposeMetrics.space12)
    }

    private func tabButton(_ tab: TerminologyTab) -> some View {
        let isSelected = activeTab == tab
        let accent = Color(nsColor: VibeComposePalette.brandBlue)
        return Button {
            activeTab = tab
        } label: {
            HStack(spacing: VibeComposeMetrics.space6) {
                ...
                if tab == .packs, enabledPackCount > 0 {
                    Text("\(enabledPackCount)")
                        .font(VibeComposeTypography.micro(.semibold))
                        .monospacedDigit()
                        .padding(.horizontal, VibeComposeMetrics.space6)
                        .padding(.vertical, 1)
                        .background(
                            (isSelected ? accent : Color.secondary)
                                .opacity(isSelected ? 0.20 : 0.14),
            ...
            .background(
                accent.opacity(isSelected ? 0.12 : 0),
                in: RoundedRectangle(
            ...
            .overlay {
                ...
                .stroke(
                    accent.opacity(isSelected ? 0.28 : 0),
                    lineWidth: 1
                )
            }
    ...
    private var enabledPackCount: Int {
        config.terminologyPacks.enabledPackIDs.count
    }
    ...
    private func entryRow(_ entry: TerminologyEntry) -> some View {
        let tint = Color(
            nsColor: entry.type == .correction
                ? VibeComposePalette.brandSpectrumViolet
                : VibeComposePalette.brandBlue
        )
    ...
    private func exportDictionary() {
        ...
        panel.nameFieldStringValue = "VibeCompose-Terminology.csv"
    ...
    private var packsTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: VibeComposeMetrics.space20) {
                ...
                ForEach(TerminologyPackCatalog.grouped, id: \.category) { group in
                    VStack(alignment: .leading, spacing: VibeComposeMetrics.space10) {
                        Text(L10n.text(group.category))
                            .font(VibeComposeTypography.caption(.semibold))
                            .foregroundStyle(.secondary)
                            .textCase(.uppercase)
                            .tracking(0.6)
                            .padding(.leading, VibeComposeMetrics.space4)

                        ForEach(group.packs) { pack in
                            TerminologyPackCard(
                                pack: pack,
                                isEnabled: config.terminologyPacks.isEnabled(pack.id),
                                conflicts: conflicts(for: pack),
                                onToggle: { setPackEnabled($0, packID: pack.id) }
                            )
                        }
                    }
                }
    ...
    private var packsIntro: some View {
        VStack(alignment: .leading, spacing: VibeComposeMetrics.space6) {
            Text(L10n.text("Add Terminology Packs"))
                .font(VibeComposeTypography.title2())
            Text(
                L10n.text(
                    "Turn on a curated pack to teach dictation the vocabulary of a field. Your personal terms always win over any pack."
                )
            )
    ...
    private func conflicts(
        for pack: TerminologyPackDefinition
    ) -> [TerminologyPackConflict] {
        TerminologyPackResolver().conflicts(
            personalEntries: config.transcription.terminology.entries,
            pack: pack
        )
    }

    private func setPackEnabled(_ enabled: Bool, packID: String) {
        let previousConfig = config
        config.terminologyPacks.setEnabled(enabled, packID: packID)
        // Enabling a pack is meaningless unless terminology itself is active.
        if enabled {
            config.transcription.terminology.enabled = true
        }
        if !persist() {
...
/// Groups the built-in packs by category for the Packs tab, preserving the
/// registry's declaration order within each category.
private enum TerminologyPackCatalog {
    struct Group {
        let category: String
        let packs: [TerminologyPackDefinition]
    }

    static let grouped: [Group] = {
        var order: [String] = []
        var buckets: [String: [TerminologyPackDefinition]] = [:]
        for pack in TerminologyPackRegistry.builtIn {
            if buckets[pack.category] == nil {
                order.append(pack.category)
            }
            buckets[pack.category, default: []].append(pack)
        }
        return order.map { Group(category: $0, packs: buckets[$0] ?? []) }
    }()
}

/// A single domain pack presented as a premium, self-describing card: icon
/// well, name, risk + term-count metadata, an enable switch, and an optional
/// conflict disclosure. Purely presentational — all mutation flows back through
/// `onToggle` so the parent keeps the single source of truth for `config`.
private struct TerminologyPackCard: View {
    let pack: TerminologyPackDefinition
    let isEnabled: Bool
    let conflicts: [TerminologyPackConflict]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: VibeComposeMetrics.space10) {
            ...
            if pack.risk == .high {
                calloutLabel(
                    ...
                    tint: Color(nsColor: VibeComposePalette.amber)
                )
            }
        ...
        .shadow(
            color: isEnabled ? accent.opacity(0.12) : .clear,
            radius: 10,
            y: 4
        )
        .animation(VibeComposeMotion.standardSpring, value: isEnabled)
    }
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: VibeComposeMetrics.space12) {
            VibeComposeIconWell(
                ...
                symbolSize: 17,
                tint: accent,
                fillOpacity: isEnabled ? 0.16 : 0.09
            )
            ...
            VStack(alignment: .leading, spacing: VibeComposeMetrics.space4) {
                ...
                HStack(spacing: VibeComposeMetrics.space6) {
                    ...
                    Text(L10n.format("%ld terms", pack.entries.count))
                        .font(VibeComposeTypography.caption())
                        .foregroundStyle(.secondary)
                    if !conflicts.isEmpty {
                        Text("·")
                            .foregroundStyle(.tertiary)
                        Text(L10n.format("%ld overlap", conflicts.count))
            ...
            Toggle(
                "",
                isOn: Binding(get: { isEnabled }, set: { onToggle($0) })
            )
            .labelsHidden()
            .toggleStyle(.switch)
            .accessibilityLabel(pack.localizedName)
        }
    }
    ...
    private var riskChipKind: VibeComposeStatusChip.Kind {
        switch pack.risk {
        ...
        VStack(alignment: .leading, spacing: VibeComposeMetrics.space6) {
            Button {
                withAnimation(VibeComposeMotion.snappySpring) {
                    showsConflicts.toggle()
                }
            } label: {
                HStack(spacing: VibeComposeMetrics.space6) {
                    Image(systemName: "chevron.right")
                        .font(.system(size: 10, weight: .bold))
                        .rotationEffect(.degrees(showsConflicts ? 90 : 0))
                    Text(
                        L10n.format(
                            "%ld overlap with your terms — yours win",
                            conflicts.count
                        )
                    )
                    .font(VibeComposeTypography.caption(.medium))
            ...
            if showsConflicts {
                ...
                .padding(.leading, VibeComposeMetrics.space16)
            }
        }
        .padding(.top, VibeComposeMetrics.space4)
