...
extension OverlayControlling {
    ...
    func showResult(text: String, outcome: InjectionOutcome) {
        showResult(text: text, outcome: outcome, autoHide: true)
    }

    func showConfirmation(title: String) {
        showConfirmation(title: title, autoHide: true)
    }
...
enum OverlaySnapshotError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingContentView:
            return "The VibeCompose HUD has no content view to capture."
        case .bitmapUnavailable:
            return "The VibeCompose HUD could not create a bitmap snapshot."
        case .pngEncodingFailed:
            return "The VibeCompose HUD could not encode its snapshot as PNG."
        }
    }
}
...
final class OverlayController: OverlayControlling, OverlaySnapshotCapturing {
    ...
    func showResult(text: String, outcome: InjectionOutcome, autoHide: Bool = true) {
        _ = text
        stopProcessingAnimation()
    ...
    func showConfirmation(title: String, autoHide: Bool = true) {
    ...
    private func apply(state: OverlayVisualState) {
        ...
        case .recording(let levels, _):
            ...
            waveformView.update(
                accentColor: VibeComposePalette.hudRecordingAccent
            )
            leadingBadgeView.isHidden = true
            iconView.isHidden = true
            setAccentRing(VibeComposePalette.hudRecordingAccent)
        case .processing:
            // Processing: warmer amber pulse, distinct from live recording blue.
            waveformView.isHidden = false
            waveformView.update(levels: displayedLevels)
            waveformView.update(
                accentColor: VibeComposePalette.hudProcessingAccent
            )
            leadingBadgeView.isHidden = true
            iconView.isHidden = true
            setAccentRing(VibeComposePalette.hudProcessingAccent)
        case .success(let kind):
            ...
            case .inserted, .pasteSent, .confirmation:
                ...
                configureBadge(
                    ...
                    tintColor: VibeComposePalette.success
                )
            case .copied:
                configureBadge(
                    symbolName: "doc.on.clipboard.fill",
                    tintColor: VibeComposePalette.amber
                )
            }
        case .error, .retryableError:
            ...
            configureBadge(
                ...
                tintColor: VibeComposePalette.error
            )
        }
    ...
    private func configureViews() {
        ...
        titleLabel.textColor = VibeComposePalette.hudText
        ...
        trailingTimerLabel.textColor = VibeComposePalette.hudText
        ...
        closeButton.contentTintColor = VibeComposePalette.hudTextMuted.withAlphaComponent(0.86)
        ...
        retryButton.contentTintColor = VibeComposePalette.hudTextMuted.withAlphaComponent(0.88)
    ...
    private func updateAccessibilityAppearance() {
        ...
        if let surfaceView = backgroundView as? OverlayHUDSurfaceView {
            surfaceView.updateChrome(
                ...
                color: VibeComposePalette.hudText.withAlphaComponent(
        ...
        closeButton.contentTintColor = VibeComposePalette.hudTextMuted.withAlphaComponent(
            accessibilityAppearance.cancelControlOpacity
        )
        retryButton.contentTintColor = VibeComposePalette.hudTextMuted.withAlphaComponent(
    ...
    private static func resolvedAccessibilityDisplayOptions(
        ...
        return AccessibilityDisplayOptions(
            ...
            differentiateWithoutColor:
                options.differentiateWithoutColor
        )
    }
    ...
    OverlayInteractiveSurface
{
    ...
    init(
        ...
        elevation.layer?.shadowOpacity =
            VibeComposeFloatingChrome.capsuleShadowOpacity
        elevation.layer?.shadowRadius =
            VibeComposeFloatingChrome.capsuleShadowRadius
        elevation.layer?.shadowOffset = CGSize(
            width: 0,
            height: VibeComposeFloatingChrome.capsuleShadowOffsetY
        )
...
private final class OverlayWaveformView: NSView {
    ...
    var debugAccentKind: OverlayDebugSnapshot.WaveformAccentKind {
        ...
        let recording = VibeComposePalette.hudRecordingAccent
            .usingColorSpace(.sRGB)
            ?? VibeComposePalette.hudRecordingAccent
        let processing = VibeComposePalette.hudProcessingAccent
            .usingColorSpace(.sRGB)
            ?? VibeComposePalette.hudProcessingAccent
    ...
    private func applyBarPresentation(animated: Bool) {
        ...
        let activeLevels = levels.isEmpty
            ...
            : levels
        let fill = accentColor ?? VibeComposePalette.hudText
        ...
        for (index, bar) in barLayers.enumerated() {
            ...
            if animated, duration > 0 {
                ...
                bar.add(boundsAnim, forKey: "vibecompose.waveform.bounds")
                bar.add(positionAnim, forKey: "vibecompose.waveform.position")
                bar.add(opacityAnim, forKey: "vibecompose.waveform.opacity")
            } else {
                CATransaction.begin()
                CATransaction.setDisableActions(true)
