...
enum SupportDiagnosticsExportError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .destinationIsDirectory(let name):
            return L10n.format(
                "VibeCompose cannot replace the folder named %@ with a diagnostics archive.",
                name
            )
        case .archiveCreationFailed:
            return L10n.text("VibeCompose could not create the diagnostics archive.")
        case .archiveOutputMissing:
            return L10n.text("VibeCompose created no diagnostics archive.")
        }
    }
}
...
struct SupportDiagnosticsExporter {
    ...
    static func suggestedFileName(now: Date = Date()) -> String {
        ...
        return "VibeCompose-Support-\(formatter.string(from: now)).zip"
    }
    ...
    func export(
        ...
        let workspaceURL = temporaryDirectoryURL
            .appendingPathComponent(
                "vibecompose-support-\(UUID().uuidString)",
                isDirectory: true
            )
        let bundleURL = workspaceURL
            .appendingPathComponent("VibeCompose-Support", isDirectory: true)
        let temporaryArchiveURL = workspaceURL
            .appendingPathComponent("VibeCompose-Support.zip")
        defer {
        ...
        let stagedDestinationURL = destinationURL
            ...
            .appendingPathComponent(
                ".vibecompose-support-\(UUID().uuidString).zip"
            )
        defer {
    ...
    VibeCompose Support Diagnostics
