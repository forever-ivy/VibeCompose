    ...
    OverlaySnapshotCapturing
{
    ...
    /// Protocol requirement — must be the 3-arg witness. A 2-arg overload is
    /// not used for `any OverlayControlling` dispatch; the protocol extension
    /// would otherwise call the no-op default and leave "Processing" stuck.
    func showResult(
        text: String,
        outcome: InjectionOutcome,
        autoHide: Bool
    ...
    private func render(
        ...
        case .hidden:
            ...
            if case .confirmation(let title) = presentation {
                blueSignalFrame.hide()
                deactivateEscapeCancellation()
                refinedHUD.showConfirmation(title: title, autoHide: false)
            } else {
                refinedHUD.hide()
                blueSignalFrame.hide()
    ...
    private func renderRefinedHUD(
        ...
        case .error(let message):
            refinedHUD.showError(message, autoHide: false)
