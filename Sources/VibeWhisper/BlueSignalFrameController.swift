    ...
    Equatable
{
    ...
    var tint: NSColor {
        ...
        case .recording, .processing:
            return VibeWhisperPalette.signalBlue
        case .success:
            return VibeWhisperPalette.success
        case .copied:
            return VibeWhisperPalette.amber
        case .error:
            return VibeWhisperPalette.error
        }
    }
    ...
    OverlaySnapshotCapturing
{
    ...
    /// (Electron, some games). Never targets VibeWhisper's own windows.
    ...
    /// excluding VibeWhisper itself, menu bar extras, and tiny UI chips.
...
private final class AIActivityGlowView: NSView {
    ...
    func playDisappearance(reduceMotion: Bool) {
        ...
        ringLayer.add(scale, forKey: "vibewhisper.edgeGlow.exitScale")
        ringLayer.add(fade, forKey: "vibewhisper.edgeGlow.exitFade")
        ringLayer.opacity = 0
    }
    ...
    // VibeWhisper retunes it toward system ambient light (soft bloom, monochrome
    ...
    private func configureReveal(thenContinuous: Bool) {
        ...
        ringLayer.add(scale, forKey: "vibewhisper.edgeGlow.revealScale")
        ringLayer.add(ringFade, forKey: "vibewhisper.edgeGlow.revealFade")
        ...
        for layersForSegment in segmentLayers {
            for (styleIndex, shapeLayer) in layersForSegment.enumerated() {
                ...
                shapeLayer.add(
                    layerFade,
                    forKey: "vibewhisper.edgeGlow.revealLayer"
                )
            }
        }
    ...
    private func addAmbientBreathing(
        ...
        for layersForSegment in segmentLayers {
            for styleIndex in 0..<animatedLayerCount {
                ...
                shapeLayer.add(
                    breathe,
                    forKey: "vibewhisper.edgeGlow.ambientBreathing"
                )
                ...
                shapeLayer.add(
                    colorBreathing,
                    forKey: "vibewhisper.edgeGlow.ambientColorBreathing"
                )
            }
        }
    }
    ...
    private func addPaletteTransition(from colors: [NSColor]) {
        ...
        for (segmentIndex, layersForSegment) in segmentLayers.enumerated() {
            ...
            for (styleIndex, shapeLayer) in layersForSegment.enumerated() {
                ...
                shapeLayer.add(
                    transition,
                    forKey: "vibewhisper.edgeGlow.paletteCrossfade"
                )
            }
        }
    }
    ...
    private func addSettleTransition(beginDelay: CFTimeInterval = 0) {
        ...
        ringLayer.add(
            scaleBloom,
            forKey: "vibewhisper.edgeGlow.settleScaleBloom"
        )
        ...
        ringLayer.add(
            swell,
            forKey: "vibewhisper.edgeGlow.settle"
        )
    }
