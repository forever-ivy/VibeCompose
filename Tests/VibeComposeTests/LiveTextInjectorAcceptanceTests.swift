...
func liveTextInjectorPastesIntoLaunchAppEditorAfterFocusMovesAway() async throws {
    guard ProcessInfo.processInfo.environment["VIBECOMPOSE_LIVE_TEXTINJECTOR_ACCEPTANCE"] == "1" else {
        return
    }

    let phrase = "focus paste test \(UUID().uuidString.prefix(8))"
    let fileURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("vibecompose-live-inject-\(UUID().uuidString).txt")
...
func liveTextInjectorPastesIntoCodexComposerAfterFocusMovesAway() async throws {
    guard ProcessInfo.processInfo.environment["VIBECOMPOSE_LIVE_CODEX_ACCEPTANCE"] == "1" else {
        return
    }
