...
func voiceModeConfigDropsInvalidAndDuplicateDecodedRules() throws {
    ...
    // Legacy "agentPlan" raw values migrate to Code Prompt on decode.
    #expect(decoded.defaultMode == .codePrompt)
...
/// The legacy Voice Mode fallback copies duplicate the built-in package
/// bodies by design (they only serve when the catalog cannot load). Lock
/// them to the packages so a prompt revision cannot leave the fallback
/// behind.
@Test
func voiceModeFallbackPromptsMatchBuiltInPackageBodies() throws {
    for mode in DictationMode.allCases {
        let definition = try #require(
            SkillRegistry.builtIn.definition(id: mode.skillID),
            "missing built-in package for \(mode.rawValue)"
        )
        #expect(
            mode.fallbackPromptInstruction == definition.promptInstruction,
            "\(mode.rawValue): fallback prompt drifted from the package body"
        )
    }
}
