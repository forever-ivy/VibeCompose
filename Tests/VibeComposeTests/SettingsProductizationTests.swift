...
/// Preferences UI now spans the window shell + root SwiftUI file after ARCH-002.
private func settingsPreferencesSources(
    relativeTo file: StaticString = #filePath
) throws -> String {
    // Tests/VibeComposeTests/<file> → repo root is three levels up.
    let root = URL(fileURLWithPath: "\(file)")
        .deletingLastPathComponent()
        .deletingLastPathComponent()
        .deletingLastPathComponent()
    let paths = [
        "Sources/VibeCompose/PreferencesWindowController.swift",
        "Sources/VibeCompose/PreferencesRootView.swift",
    ]
...
func standardManagementWindowsSupportDockMinimization() throws {
    ...
    let mainSource = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/main.swift"
        ),
        encoding: .utf8
    )
    #expect(
    ...
    let settingsSource = try settingsPreferencesSources()
...
func settingsWindowStateRestoresLastPaneAndHonorsDeepLinks() throws {
    let suiteName = "VibeComposeTests.Settings.\(UUID().uuidString)"
...
func skillRulesLiveOnDedicatedSettingsPaneNotInstall() throws {
    ...
    let librarySource = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/SkillLibraryWindowController.swift"
        ),
        encoding: .utf8
    )
    let preferencesSource = try settingsPreferencesSources()
    ...
    #expect(librarySource.contains("VibeComposePaneHeader(title: L10n.text(\"Rules\")"))
    ...
    #expect(librarySource.contains("vibeComposeInset("))
...
func skillShowcaseMorphUsesStableInterruptibleGeometry() throws {
    ...
    #expect(player.contains("VibeComposeMotion.showcaseMorph"))
...
func settingsWindowFallsBackFromUnknownPersistedPane() throws {
    let suiteName = "VibeComposeTests.Settings.Unknown.\(UUID().uuidString)"
    let defaults = try #require(UserDefaults(suiteName: suiteName))
    defer {
        defaults.removePersistentDomain(forName: suiteName)
    }
...
func settingsWindowStatePersistsSkillLibrarySection() throws {
    let suiteName =
        "VibeComposeTests.Settings.SkillLibrary.\(UUID().uuidString)"
    let defaults = try #require(UserDefaults(suiteName: suiteName))
    defer {
        defaults.removePersistentDomain(forName: suiteName)
    }
...
func settingsWindowStateNormalizesPersistedPaneOnSave() throws {
    let suiteName =
        "VibeComposeTests.Settings.Normalize.\(UUID().uuidString)"
    let defaults = try #require(UserDefaults(suiteName: suiteName))
    defer {
        defaults.removePersistentDomain(forName: suiteName)
    }
...
func `Skill Library deep links stay inside the Settings window`() throws {
    ...
    let stateSource = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/SettingsWindowState.swift"
        ),
        encoding: .utf8
    )
...
func transientPanelsRestoreHostFrontmostInsteadOfSettings() throws {
    ...
    let injectorSource = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/TextInjector.swift"
        ),
        encoding: .utf8
    )
    ...
    // Capture + restore helpers exist and refuse to re-activate VibeCompose.
    #expect(
    ...
    // Shared presentation context resolves the restore target once, before
    // either panel activates VibeCompose.
    let transientContextSource = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/TransientPanelContext.swift"
        ),
        encoding: .utf8
    )
    #expect(
        transientContextSource.contains(
            "externalFrontmostForTransientRestore"
        )
    )
    ...
    #expect(
        switcherSource.contains(
            "TransientPanelPresentationContext.capture()"
        )
    )
    #expect(
        switcherSource.contains(
            "presentationContext.restoreTarget"
        )
    )
    #expect(
        switcherSource.contains(
...
func settingsSourceKeepsTheMacOSSidebarAndAutosavingContract() throws {
    let source = try settingsPreferencesSources()
    ...
    #expect(source.contains("vibeComposeFloatingSidebarGlass("))
    ...
    #expect(source.contains("VibeComposeFloatingChrome.sidebarCornerRadius"))
    ...
    #expect(source.contains("VibeComposePalette.sidebarSelectionBackground"))
    #expect(source.contains("VibeComposePalette.sidebarSelectionForeground"))
    ...
    #expect(!source.contains("VibeComposeWindowBackdrop"))
    ...
    #expect(source.contains("VibeComposeSidebarSymbol("))
    #expect(!source.contains("VibeComposeSidebarIconWell("))
    ...
    #expect(!source.contains("Skills require VibeCompose Pro"))
    ...
    #expect(source.contains("case .terminology:"))
    #expect(source.contains("TerminologyManagerView("))
    #expect(source.contains("isEmbedded: true"))
    ...
    #expect(!source.contains("Choose how VibeCompose transcribes"))
...
func liquidGlassStaysInSystemNavigationChrome() throws {
    ...
    let history = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/HistoryWindowController.swift"
        ),
        encoding: .utf8
    )

    #expect(!visualSystem.contains("VibeComposeWindowBackdrop"))
    #expect(!visualSystem.contains("LinearGradient("))
    #expect(!visualSystem.contains("RadialGradient("))
    #expect(visualSystem.contains("VibeComposeSecondaryButtonStyle: PrimitiveButtonStyle"))
    #expect(visualSystem.contains(".buttonStyle(.glass)"))
    #expect(visualSystem.contains(".buttonStyle(.glassProminent)"))
    #expect(visualSystem.contains("VibeComposeFloatingSidebarChrome"))
    #expect(visualSystem.contains("vibeComposeFloatingSidebarGlass"))
...
func embeddedManagementSurfacesDoNotForceTheSettingsDetailWiderThanItsWindow() throws {
    ...
    for relativePath in [
        "Sources/VibeCompose/HistoryWindowController.swift",
        "Sources/VibeCompose/TerminologyWindowController.swift",
    ] {
        let source = try String(
...
func primaryProductSurfacesDoNotRenderExplanatorySmallPrint() throws {
    ...
    let sources = try [
        ...
        try String(
            contentsOf: root
                .appendingPathComponent("Sources/VibeCompose")
    ...
    // Empty states render detail as visible secondary copy (not only an AX hint).
    #expect(sources.contains("if let detail, !detail.isEmpty"))
...
func statusMenuExposesSoftwareUpdateEntry() throws {
    ...
    let source = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/StatusMenuController.swift"
        ),
        encoding: .utf8
    )
