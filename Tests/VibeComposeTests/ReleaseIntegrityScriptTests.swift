...
func strictEnvironmentLoaderAcceptsDataWithoutExecutingIt() throws {
    ...
    let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("VibeComposeReleaseTests-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: temporaryDirectory, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: temporaryDirectory) }

    let safeURL = temporaryDirectory.appendingPathComponent("safe.env")
    try "VIBECOMPOSE_APP_NAME=VibeCompose\n".write(
        to: safeURL,
        atomically: true,
        encoding: .utf8
    )
    ...
    let safeResult = try runBash(
        """
        source "$1"
        load_env_file "$2" VIBECOMPOSE_APP_NAME
        printf '%s' "$VIBECOMPOSE_APP_NAME"
        """,
        arguments: [loaderURL.path, safeURL.path]
    )
    #expect(safeResult.status == 0)
    #expect(safeResult.stdout == "VibeCompose")
    ...
    try "VIBECOMPOSE_APP_NAME=$(touch \(markerURL.path))\n".write(
        to: maliciousURL,
        atomically: true,
        encoding: .utf8
    )
...
func packagingAndInstallationScriptsKeepReleaseIntegrityFailClosed() throws {
    ...
    #expect(packageScript.contains("VIBECOMPOSE_REQUIRE_DEVELOPER_ID"))
    #expect(packageScript.contains("VIBECOMPOSE_BUILD_CONFIGURATION"))
    #expect(packageScript.contains("--configuration \"$BUILD_CONFIGURATION\""))
    #expect(
        packageScript.contains(
            "Developer ID release packaging requires VIBECOMPOSE_BUILD_CONFIGURATION=release."
        )
    )
    ...
    #expect(packageScript.contains("VIBECOMPOSE_NOTARY_KEYCHAIN"))
    ...
    #expect(packageScript.contains("VIBECOMPOSE_SPARKLE_FEED_URL"))
    #expect(packageScript.contains("VIBECOMPOSE_SPARKLE_PUBLIC_ED_KEY"))
    #expect(packageScript.contains("VIBECOMPOSE_CAPABILITY_POLICY_URL"))
    #expect(packageScript.contains("VIBECOMPOSE_CAPABILITY_PUBLIC_ED_KEY"))
    #expect(!packageScript.contains("VIBECOMPOSE_LICENSE_PUBLIC_ED_KEY"))
    #expect(!packageScript.contains("VIBECOMPOSE_PRO_PREVIEW_ENABLED"))
    ...
    #expect(packageScript.contains("strip_codesign_hostile_xattrs"))
    #expect(packageScript.contains("xattr -cr \"$app\""))
    #expect(
        packageScript.contains(
            "strip_codesign_hostile_xattrs \"$DIST_APP_DIR\""
        )
    )
    #expect(packageScript.contains("DIST_VERIFY_APP="))
    #expect(packageScript.contains("\"$DIST_VERIFY_APP\" >/dev/null"))
...
func releaseMetadataGeneratorProducesExactArtifactContract() throws {
    ...
    let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "VibeComposeReleaseMetadataTests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    let zipURL = temporaryDirectory.appendingPathComponent("VibeCompose.zip")
    let dmgURL = temporaryDirectory.appendingPathComponent("VibeCompose.dmg")
    ...
    let result = try runBash(
        ...
        swift "$1" \
          --output "$2" \
          --app-name VibeCompose \
          --bundle-id app.vibecompose.mac \
          ...
          --zip "$3" \
          --dmg "$4" \
          --zip-url https://example.invalid/VibeCompose.zip \
          --dmg-url https://example.invalid/VibeCompose.dmg \
    ...
    let insecureResult = try runBash(
        ...
        swift "$1" \
          ...
          --zip "$3" \
          --dmg "$4" \
          --zip-url http://example.invalid/VibeCompose.zip \
          --dmg-url https://example.invalid/VibeCompose.dmg
        """,
        arguments: [
            generatorURL.path,
...
func sparkleAppcastVerifierMatchesManifestArchiveAndPublicKey() throws {
    ...
    let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "VibeComposeSparkleVerificationTests-\(UUID().uuidString)",
            isDirectory: true
        )
    try FileManager.default.createDirectory(
        at: temporaryDirectory,
    ...
    let archiveURL = temporaryDirectory.appendingPathComponent(
        "VibeCompose-0.1.0-macos-arm64.zip"
    )
    ...
    let archiveData = Data("signed-vibecompose-update".utf8)
    ...
    let sha256 = SHA256.hash(data: archiveData)
        ...
        .joined()
    let downloadURL = "https://example.invalid/VibeCompose-0.1.0-macos-arm64.zip"
...
func releaseScriptsKeepCaskAndUpdaterGateFailClosed() throws {
    ...
    let cask = try String(
        contentsOf: root.appendingPathComponent(
            "packaging/homebrew/Casks/vibecompose.rb"
        ),
        encoding: .utf8
    )
    ...
    #expect(updateCaskScript.contains("VIBECOMPOSE_CASK_OUTPUT_PATH"))
    #expect(metadataScript.contains("VIBECOMPOSE_RELEASE_BASE_URL"))
    #expect(metadataScript.contains("VIBECOMPOSE_REQUIRE_DEVELOPER_ID"))
    #expect(
    ...
    #expect(releaseGate.contains("VIBECOMPOSE_SPARKLE_APPCAST_PATH"))
    #expect(releaseGate.contains("OWCapabilityPolicyURL"))
    #expect(releaseGate.contains("OWCapabilityPublicEDKey"))
    #expect(releaseGate.contains("VIBECOMPOSE_CAPABILITY_POLICY_PATH"))
    ...
    #expect(releaseGate.contains("VIBECOMPOSE_RELEASE_BASE_URL"))
    ...
    #expect(signedRelease.contains("VIBECOMPOSE_BUILD_CONFIGURATION=release"))
...
func homebrewCaskUpdaterWritesManifestURLAndChecksumTogether() throws {
    ...
    let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "VibeComposeCaskTests-\(UUID().uuidString)",
            isDirectory: true
        )
    try FileManager.default.createDirectory(
        at: temporaryDirectory,
    ...
    let outputURL = temporaryDirectory.appendingPathComponent("vibecompose.rb")
    let checksum = String(repeating: "a", count: 64)
    let downloadURL = "https://downloads.example.com/vibecompose/v0.1.0/VibeCompose-0.1.0-macos-arm64.zip"
    let manifest: [String: Any] = [
    ...
    let result = try runBash(
        """
        VIBECOMPOSE_RELEASE_MANIFEST_PATH="$2" \
        VIBECOMPOSE_CASK_OUTPUT_PATH="$3" \
        "$1"
        """,
...
@Test
func swiftTestOutputVerifierFailsClosedOnRecordedSwiftTestingIssues() throws {
    let root = repositoryRoot()
    let verifierURL = root.appendingPathComponent(
        "scripts/verify_swift_test_output.py"
    )
    let checkURL = root.appendingPathComponent("scripts/check.sh")
    let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "VibeComposeSwiftTestVerifier-\(UUID().uuidString)",
            isDirectory: true
        )
    try FileManager.default.createDirectory(
        at: temporaryDirectory,
    ...
    let cleanLogURL = temporaryDirectory.appendingPathComponent("clean.log")
    try Data(
        """
        Test run started.
        Test example() passed after 0.001 seconds.
        Test run with 1 test in 0 suites passed after 0.002 seconds.
        """.utf8
    ).write(to: cleanLogURL)
    let issueLogURL = temporaryDirectory.appendingPathComponent("issue.log")
    try Data(
        """
        Test example() recorded an issue at ExampleTests.swift:12:5: Expectation failed.
        Test example() failed after 0.001 seconds with 1 issue.
        Test run with 1 test in 0 suites failed after 0.002 seconds.
        """.utf8
    ).write(to: issueLogURL)

    let cleanResult = try runBash(
        #"python3 "$1" --command-status 0 "$2""#,
        arguments: [verifierURL.path, cleanLogURL.path]
    )
    #expect(cleanResult.status == 0)

    let issueResult = try runBash(
        #"python3 "$1" --command-status 0 "$2""#,
        arguments: [verifierURL.path, issueLogURL.path]
    )
    #expect(issueResult.status != 0)
    #expect(issueResult.stderr.contains("recorded an issue"))

    let commandFailureResult = try runBash(
        #"python3 "$1" --command-status 1 "$2""#,
        arguments: [verifierURL.path, cleanLogURL.path]
    )
    #expect(commandFailureResult.status != 0)
    #expect(commandFailureResult.stderr.contains("status 1"))

    let checkSource = try String(contentsOf: checkURL, encoding: .utf8)
    #expect(checkSource.contains("verify_swift_test_output.py"))
    #expect(checkSource.contains("PIPESTATUS[0]"))
    #expect(checkSource.contains("tee \"$SWIFT_TEST_LOG\""))
    #expect(checkSource.contains("chmod 600 \"$SWIFT_TEST_LOG\""))
}
