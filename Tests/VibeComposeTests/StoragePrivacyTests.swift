...
func transcriptionHistoryAppliesAgeAndCountRetention() throws {
    let root = temporaryVibeComposeDirectory()
...
func transcriptionHistoryPersistsRedactedSkillRunReceipt() throws {
    ...
    let receipt = SkillRunReceipt(
        ...
        skillID: "com.vibecompose.direct",
        displayName: "Direct",
        source: "builtin",
        version: "1.0.0",
        revision: "1.0.0",
...
@Test
func storageCleanupDeletesOnlyTheVibeComposeContainer() throws {
    ...
    try Data("legacy-private".utf8).write(
        to: legacyRoot.appendingPathComponent("transcription-history.jsonl")
    )
...
@Test
func storageCleanupRejectsSymlinkedPreRenameContainerBeforeDeletingAnything() throws {
    let parent = FileManager.default.temporaryDirectory
        .appendingPathComponent(UUID().uuidString, isDirectory: true)
    ...
    let currentSentinel = root.appendingPathComponent("current.txt")
    let outsideSentinel = outside.appendingPathComponent("outside.txt")
    try FileManager.default.createDirectory(
        at: root,
        withIntermediateDirectories: true
    )
    ...
    try Data("keep-current".utf8).write(to: currentSentinel)
    try Data("keep-outside".utf8).write(to: outsideSentinel)
    try FileManager.default.createSymbolicLink(
        at: legacyRoot,
        withDestinationURL: outside
    )
    ...
    #expect(throws: StorageCleanupError.symbolicLink(legacyRoot.path)) {
        try StorageCleanupService(
            applicationSupportURL: root
        ).deleteAllData()
    }
    #expect(FileManager.default.fileExists(atPath: currentSentinel.path))
...
func temporaryArtifactCleanupRemovesOnlyOwnedCrashOrphans() throws {
    ...
    let recordingName = "vibecompose-\(UUID().uuidString).wav"
    let uploadName = "vibecompose-upload-\(UUID().uuidString).multipart"
    let symlinkName = "vibecompose-\(UUID().uuidString).wav"
    ...
    let lookalikeURL = root.appendingPathComponent("vibecompose-not-a-uuid.wav")
    let directoryLookalike = root.appendingPathComponent(
        "vibecompose-\(UUID().uuidString).wav",
        isDirectory: true
    )
    let outsideTarget = root.deletingLastPathComponent()
        .appendingPathComponent("vibecompose-cleanup-sentinel-\(UUID().uuidString)")
...
private func temporaryVibeComposeDirectory() -> URL {
