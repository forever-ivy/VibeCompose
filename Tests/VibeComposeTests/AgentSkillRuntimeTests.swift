...
private func writeAgentSkill(
    ...
    let skill = [
        "---",
        "name: \(name)",
        "description: A portable Agent Skill used by VibeCompose tests",
    ...
    if let profile {
        try Data(profile.utf8).write(
            to: directory.appendingPathComponent("vibecompose.yaml")
        )
    }
...
@Test
func vibeComposeProfileRejectsUnknownNestedKeys() throws {
    let root = try agentSkillTestRoot()
    defer { try? FileManager.default.removeItem(at: root) }
    let url = try writeAgentSkill(
        at: root,
