...
func simplifiedChineseLocalizationCoversCorePermissionAndDictationUI() throws {
    ...
    #expect(strings["VibeCompose Settings"] == "VibeCompose 设置")
    ...
    #expect(
        ...
        ] == "VibeCompose 仍无法确认麦克风权限。请点击“刷新权限状态”或重新打开应用。"
    )
    ...
    #expect(strings["Skills require VibeCompose Pro"] == nil)
...
func simplifiedChineseInfoPlistLocalizesMicrophoneUsageDescription() throws {
    let root = URL(fileURLWithPath: #filePath)
        ...
        .deletingLastPathComponent()
    let stringsURL = root
        .appendingPathComponent("Sources/VibeCompose/Resources/zh-Hans.lproj/InfoPlist.strings")
    let data = try Data(contentsOf: stringsURL)
    let propertyList = try PropertyListSerialization.propertyList(from: data, format: nil)
    let strings = try #require(propertyList as? [String: String])
...
@Test
func simplifiedChineseLocalizationCoversSkillBrowseTaxonomy() throws {
    let root = URL(fileURLWithPath: #filePath)
        .deletingLastPathComponent()
        .deletingLastPathComponent()
        .deletingLastPathComponent()
    ...
    #expect(strings["Dictation"] == "听写")
    #expect(strings["Rewrite"] == "改写")
    #expect(strings["Developer"] == "开发")
    #expect(strings["Documents"] == "文档")
