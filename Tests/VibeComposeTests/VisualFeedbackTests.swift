    ...
    throws
{
    ...
    let selection = try #require(
        VibeComposePalette.sidebarSelectionLightColor
            .usingColorSpace(.sRGB)
    )
...
func appDelegateUsesUnifiedFeedbackSurfaceAtRuntime() throws {
    ...
    let source = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeCompose/AppDelegate.swift"
        ),
        encoding: .utf8
    )
...
func activityGlowKeepsTheCoreSteadyWhileAmbientLayersBreatheTogether()
{
    ...
    #expect(
        initial.animationKeys.contains(
            "vibecompose.edgeGlow.ambientColorBreathing"
        )
    )
    // First show also plays the soft reveal bloom.
    #expect(
        initial.animationKeys.contains(
            "vibecompose.edgeGlow.revealScale"
        )
        || initial.animationKeys.contains(
            "vibecompose.edgeGlow.revealFade"
        )
    )
    #expect(
...
func activityGlowCrossfadesStateColorsWithoutMovingTheGradient()
{
    ...
    #expect(
        animationKeys.contains(
            "vibecompose.edgeGlow.paletteCrossfade"
        )
    )
    #expect(
    ...
    throws
{
    ...
    let output = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "vibecompose-edgeglow-\(UUID().uuidString).png"
        )
    defer {
        controller.hide()
