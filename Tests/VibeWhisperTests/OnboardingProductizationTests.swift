...
func onboardingStateShowsUntilTheCurrentFlowIsCompleted() throws {
    let suiteName = "VibeWhisperTests.Onboarding.\(UUID().uuidString)"
...
func privateAcceptanceDoesNotPersistOnboardingCompletion() throws {
    let suiteName =
        "VibeWhisperTests.Onboarding.Private.\(UUID().uuidString)"
    let defaults = try #require(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
...
func onboardingSourceKeepsMicrophoneRequiredAndPasteOptional() throws {
    ...
    let source = try String(
        contentsOf: root.appendingPathComponent(
            "Sources/VibeWhisper/OnboardingWindowController.swift"
        ),
        encoding: .utf8
    )
    ...
    #expect(source.contains("VibeWhisperOnboardingCTAButtonStyle"))
    #expect(
