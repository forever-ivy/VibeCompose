...
func accessibilityAcceptanceCoversEveryPrimaryProductSurface() throws {
    ...
    #expect(source.contains("settings-general"))
    ...
    #expect(!source.contains("settings-terminology"))
    ...
    #expect(
        source.contains(
            "\"skill-library-installed\" \"--open-skill-library\" \"installed\""
        )
    )
    #expect(
        source.contains(
            "\"skill-library-discover\" \"--open-skill-library\" \"discover\""
        )
    )
    #expect(
        source.contains(
            "\"skill-library-created\" \"--open-skill-library\" \"created\""
        )
    )
    #expect(source.contains("\"skill-switcher\" \"--open-skill-switcher\""))
    #expect(source.contains("\"preview\" \"--preview-demo\""))
